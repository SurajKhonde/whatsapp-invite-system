# పిlooopu — Bulk Personalized WhatsApp Invite Platform
...
పిlooopu solves this. Upload your guest list, pick a template, and every guest receives a **personalized invite with their name** — reliably, with full delivery tracking, retries on failure, and all phone numbers encrypted at rest.
...
Your guests' phone numbers are PII. పిlooopu encrypts every phone number with AES-256-CBC (unique IV per number) before storing. The plaintext is only available inside the worker at send-time, never in API responses or logs.
